...
\begin{multicols*}{2}
  ...
  \paragraph{Solution.}
  Let $g_{t}$ be the random variable which counts the gain at time step $t$ and by $G_{t} = \sum_{t}^{T}{g_{t}}$. Recall that for any pair of random variables $X, Y$ such that $X \ge Y$ holds that $\expp{X} \ge \expp{Y}$. Also notice that for $x$ restricted to some range $[-r,r]$ there are constants $c_{+}, c_{-}$ depend on $r$ such that $c_{-}x^{2} \le e^{x} - 1 - x \le c_{+}x^{2}$. Namely, the exponent is bounded by quadric approximation (second Tylor series order). By the monotonous property of the expectation, for any random variable $X$ that maps to bounded range $[-r,r]$, it holds that:
  \begin{equation*}
    \begin{split}
