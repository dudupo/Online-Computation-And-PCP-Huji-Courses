...
\paragraph{Soundness.} Suppose that $\varphi \not\in \text{E3-CNF-SAT}$, Thus for any $x \in \mathbb{F}_{2}^{*}$ no more than $\mu$ closures are satisfied. Assume throawds contradiction that more than $\mu^{\prime}= 6\mu$ of the restriction are satisfied. We will show how one can construct a vector $x \in \mathbb{F}_{2}^{*}$ which will satisfies more than $\mu$ closures.        
