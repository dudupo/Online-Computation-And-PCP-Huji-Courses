...
\section{Ex (5).}
\subsection{(a)} As shown in the above section, by the fact that $q$ is prime we have that $g_{v,u} $ act on $\FF_{q}^{m}$ by $g_{u,v}(x) = v + ux$, (for any $u\neq 0$) Thus $f(g_{u,v}(x))$ is just a permutation over the values of $f$. As the number of zeros remain the same, we have that $f(g_{u,v}(x))$ is also degree $d$ polynomial. Thus the restircted polynomail $f|_{L}$ correspond to restriction $L_{\prime}$ of another polynomial obtained by taking $v^{\prime} =0$ and $u$ to be supported only on single coordinate. Hence the restricted polynomial can has at most $d$ zeros.
\subsection{(b)}
%\end{multicols*}
  \printbibliography 
\end{document}
