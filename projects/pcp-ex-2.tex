...
\begin{proof} For convenient let's denote by $g_{i}\left( x_{m-i} \right)$ the difference that been queried in line number $4$. 
  \begin{enumerate}
    ...
    \item Soundness. Assume an adversiray input in which $f_{m} = 0$ but $\sum_{\mathbf{x} \in [d]^{m}}{f_{0}(x)}\neq 0$. (The case which $f_{m} \neq 0$ is not intersting). Now, observers that this can happens only if either at least one the $f_{i}$ dosn't setted according the definetion above or at least of the $f_{i}$ is not a polynomail at degree at most $m\cdot d $.  In the first case, there exsists at least one $i$ such that $g_{i}\neq 0$:
%      \begin{equation}
%        \label{equ:sat}
%        \begin{split}
