...
\paragraph{The reduction.} Let $\braket{G=(V,E), \{ c_{e} \} }$ be a given instance of the Label cover problem. For each edge $e = \{v,u\} \in E$ define the test $T_{\varepsilon}\left( c_{e} \right)$ as defined above when we think on the function $f,g : \pm^{|\Sigma|} \rightarrow \pm $ as the 'witness'/'input'.  and notice that above binary field .    
\begin{equation*}
  \begin{split}
    & A(v) = \Chi{i} \\ 
    & A(u) = \Chi{j} \\ 
