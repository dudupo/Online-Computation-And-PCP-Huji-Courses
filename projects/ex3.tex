...
\begin{multicols*}{2}
  ...
  \paragraph{Solution.} Let us prove the following inequality first: 

  \paragraph{Lemma.} Let $w_0, w_1, .. w_n \in [0,1]^n$ such that $\sum_{i}{w_{i}} = 1$ then there exists $\eta$ such that: 

  \begin{equation*}
    \begin{split}
      \sum_{i}{w_{i}e^{\eta x_{i}}} \le e^{\eta}e^{\sum_{i}{w_{i}x_{i}}} 
    \end{split}
  \end{equation*}

  Define the potential $\psi\left( t \right) =  \sum_{j}{ e^{\eta G_{i,t}}}$ and notice that by the fact that $e^{x}$ is positive function we have that $\psi\left( t \right) \ge e^{\eta \max_j G_{i,j}}$. In addition: 
  \begin{equation*}
    \begin{split}
