...
\le &  \sum_{|I| \text{is odd} , |I| \le \eta n  }{\alpha_{I}^{2}\beta_{C^{-1}(I)}^{2}} + \left( 1 -2 \varepsilon \right)^{2\eta n }\sum_{|I| \text{is odd} , |I| \ge \eta n  }{\alpha_{I}^{2}\beta_{C^{-1}(I)}^{2}  } 
 ...
 \begin{equation*}
   ...
   \end{split}
 \end{equation*}
 As the sumamtion is over $I$ at odd size, the empty set is not conunted in the summation, namly there must be a non empty $I$ such that $|I| \le \frac{1}{2} \log_{1-2\varepsilon}\left( \delta^{3} \right)$ and $\alpha_{I}\beta_{C^{-1}\left( I \right)}$ have positive weight. Thus we can define: 
 \begin{equation*}
   \begin{split}
