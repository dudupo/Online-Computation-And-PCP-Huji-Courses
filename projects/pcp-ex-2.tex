...
For the edges, connect between any closure on the right an edge between it and the left vertices which associate with the first terminal on the closure.Notice that the right degree of the graph is $1$ while the left degree is 

\begin{equation*}
  \begin{split}
    \sum_{x \in \text{terminals} }{ \left( 2 \times \text{ number of closures containing } x \right) }  
  \end{split}
\end{equation*}
