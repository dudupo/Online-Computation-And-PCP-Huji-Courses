...
\begin{multicols*}{2}
  ...
  \paragraph{Solution.} Define the potential $\psi\left( t \right) =  \sum_{j}{ e^{\eta G_{i,t}}}$ and notice that by the monotonies of $e^{x}$ we have that $\psi\left( t \right) \le ne^{\eta \max_j G_{i,j}}$. In addition we have that: 
  \begin{equation*}
    \begin{split}
    ...
    \end{split}
  \end{equation*}
  So after $T$ steps the potential is bounded by $\psi\left( 0 \right)e^{\eta T}$.
