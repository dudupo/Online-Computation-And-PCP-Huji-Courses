...
\begin{proof}
  Let $v \in R$ be a vertex that satisfies less than two edges. By the fact that for any 3-CNF closure there is exactly one assignment over the triple in her support does not satisfy it, it follows that $|\Sigma_{v}| = 2^{3}-1 = 7$. Therefore, we can replace $A(v)$ to a triple that agree with the first two vertices connected to it (And that process might restrict the value of the third terminal in the triple).     
\end{proof}
