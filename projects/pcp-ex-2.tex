...
\paragraph{The reduction.} Let $\braket{G=(V,E), \{ c_{e} \} }$ be a given instance of the Label cover problem. For each edge $e = \{v,u\} \in E$ define the test $T^{f,g}_{\varepsilon}\left( c_{e} \right)$ as defined above when we think on the function $f,g : \pm^{|\Sigma|} \rightarrow \pm $ as the 'witness'/'input'.  And notice that on binary field the test is equivalence for picking randomly an equations from the liner system in which each equation contains exactly tree variables.  
\begin{equation*}
  \begin{split}
    & A(v) = \Chi{i} \\ 
    & A(u) = \Chi{j} \\ 
  ...
  \end{split}
\end{equation*}
%vertex $v \in V$ define a variable $\xi_{v} \in \mathbb{F}_{2}^{|\Sigma|}$ and for any
For any $x,y,\mu, z$ 
\paragraph{Compeletnce. } .. 
\paragraph{Soundness.} .. 
\section{Part 3.}
