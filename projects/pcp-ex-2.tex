...
\section{Part 3.}
Define the Bipartite graph $G = (L,R,E)$. Associate the left vertices with the variables and the right vertices with the closures. Let $x$ be variable, and $\varphi$ a closure in which $x$ appears. Sets the permeation of the edges as following, if $x$ appears in his positive form, namely $\varphi = x \vee .. $ then set on the edge $\left\{ x, \varphi \right\}$ the identity permutation otherwise set the flipping permutation. 
