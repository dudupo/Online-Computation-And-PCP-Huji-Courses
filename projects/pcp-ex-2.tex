...
Now we are about to show that for at least $\delta$ of tests the product $ \expp{\mu f_{v}(\mu x )f_{u}(y)f_{v}(zx c^{-1}(y)) | u,v }$ conditioned on the test is greater than $\delta$. For convinent let's use the notation $\expp{\ge \delta}$ for refering to all the test that the avereing in on their product is grater than $\delta$, and by the same manner let's use the notatin $\expp{\le \delta}$. So:   

  \begin{equation*}
    \begin{split}
      2\delta \le  &  \expp{\mu f_{v}(\mu x )f_{u}(y)f_{v}(zx c^{-1}(y))} = \\ 
      & \  \prb{ u,v \text{ s.t } \expp{\ge \delta} }  \expp{\mu f_{v}(\mu x )f_{u}(y)f_{v}(zx c^{-1}(y)) | u,v \text{ s.t } \expp{\mu f_{v}(\mu x )f_{u}(y)f_{v}(zx c^{-1}(y))} \ge \delta  } +\\ 
            & \  \prb{ u,v \text{ s.t } \expp{\le \delta} } \expp{\mu f_{v}(\mu x )f_{u}(y)f_{v}(zx c^{-1}(y)) | u,v \text{ s.t } \expp{\mu f_{v}(\mu x )f_{u}(y)f_{v}(zx c^{-1}(y))} \le \delta } \\
            & \le \prb{ u,v \text{ s.t } \expp{\ge \delta} }\cdot 1 + \prb{ u,v \text{ s.t } \expp{\le \delta} } \cdot \delta \\ 
            & \le \prb{ u,v \text{ s.t } \expp{\ge \delta} } + \delta 
    \end{split}
  \end{equation*}
