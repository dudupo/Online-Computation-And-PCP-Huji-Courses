...
\paragraph{Solution.}. Let's start by formulate an integer LP for the Ski-Rental problem. Denote by $m$ the days number, assoicate a varible $x$ indecating wether or not the algorithm dicide to buy. Also let's associate with each day a varible $z_{j}$ which indicate if the algorithm pays for rent. In each turn the soultion must satisfy the restrictions $ z_{j} + x  \ge 1$. The cost which we would like to minimize is $ M\cdot x + \sum_{j}{z_{j}}$.   
