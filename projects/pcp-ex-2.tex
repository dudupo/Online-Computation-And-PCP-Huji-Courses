...
Let us denote $g_{i}\left( x_{m-i} \right)$ as the difference that is queried in line 4 for convenience.
  \begin{enumerate}
...
Completeness is easy to understand. If the assignment is honest, then by definition $g_i = 0$ for any $i \in [m]$, and thus for any $x_{m-i}$, we will have that $g_i(x_{m-i}) = 0$. Therefore, in that case, the iteration will pass, and the whole proof will be expected with probability $1$.
...
Assume an adversary input in which $f_{m} = 0$ but $\sum_{\mathbf{x} \in [d]^{m}}{f_{0}(x)}\neq 0$. This can only occur if at least one of the $f_{i}$ is not set according to the definition or at least one of the $f_{i}$ fails the $m\cdot d$-degree polynomial test with a probability greater than $1 - \frac{m\cdot d}{q}$. (The case where $f_{m} \neq 0$ is not interesting).
