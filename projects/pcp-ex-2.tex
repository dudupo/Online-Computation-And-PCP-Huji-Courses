...
\begin{proof} \ 
  ...
  \begin{enumerate}
    \item Compeletnce. The key point here is the fact that the extansion $\xi_{P} = \varphi_{P} \cdot f_{P}$ is uniqe and agrees with $\varphi\cdot f$ on all pointes in the cube $[d]^{m}$. If the proof is honest then all validity of the input check at lines $1-4$ are pass with probability $1$. Then we get by the compeletnce of the sumcheck verification that if indeed $\sum_{x \in [d]^{m}}{ \xi(x) }$ $ = \sum_{x \in [d]^{m}}{ \varphi(x)f_{P}(x) } = 0$ then that line is also pass eith probability $1$. 
    \item Soundness.  
  \end{enumerate}
\end{proof}
