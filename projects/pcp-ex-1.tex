...
\subsection{(b)} 
We will prove the claim only for monoms $x^{l} \mapsto \left( tx + s \right)^{l}$ and that will sufficent by lineratiy arguments. Again we will show correction by induction on $l$, if $l=1$ then it's trivial. Suppose the rightness of the claim for any degree lower then $l$ and consider the monom $x^{l+1}$.  Note that for any $t\neq 0,s \in \FF_{q}$ we have that $(tx +s)^{l+1} = (tx +s)^{l} \left( tx + s \right) = s (tx +s)^{l} + tx (tx +s)^{l}$. 
Now applay the induction assumption to obtain that:  
\begin{equation*}
  \begin{split}
    a_{d+1} (tx +s)^{l} (x_{d+1} ) = \sum_{i}{a_{i} (tx +s)^{l} (x_{i} )}\Rightarrow \left(tx (tx +s)^{l}\right) (x_{i}) =tx_{i}\left( a_{i}^{-1}\left(a_{d+1} - a_{j}( \cdot )  \right) \right)   
  \end{split}
\end{equation*}<++>
Note that $q$ is prime, thus $\left( \FF_{q} / 0 , \cdot  \right)$ and  $ \left( \FF_{q} , +  \right)$ are cyclic groups and therefore any $s \in \left( \FF_{q} , +  \right)$ and a subset of $J \subset \FF_{q}$ of $d+1$ coordinates the set $s + J$ is also at size $d+1$ (no collision occurs). Therefore we could find 

Now for any set $\{x_{1} + s, x_{2} + s , .. ,x_{d+1}+s \}$ we could find the interpolation coffetions relative to the function  
