...
For example, assume that $v$ associate with $x\vee \bar{y} \vee z $ and let $u_{1},u_{2}$ be the vertices associated with $x$ and $y$. Then $\Sigma_{v} = \{ 1 \}$, $\Sigma_{u_{1}}=\Sigma_{u_{2}}=\{1,0\}$ and $c_{u_{1},v}A_{u_{1}} = A_{u_{1}}$, $c_{u_{2},v}A_{u_{2}} = \bar{A_{u_{2}}}$. 

Observes that a labeling $A$ in which $c_{u,v}A_{u}=A_{v}$ correspond to an assignment in which the variable associate with $u$ satisfies the closure associated with $v$.  
