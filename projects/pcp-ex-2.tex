...
%      \end{equation}
    ...
    the test accept.
      ...
      We use similar arguments to treat the case in which one of the functions $f_{i}$ is not close to a polynomail. As it given in the question that the validily of $f_{P}$ can be assumed, we guess that intent was not to dig down into a multivariable polynomails verfication. Thus we just mention without a proof that probability for rejecting is grater than the probabiliy that $f_{i}$ fail in a low degree polynomail test and that also happens with probability at least $1 - \frac{m\cdot d}{q}$. 
  \end{enumerate}
\end{proof}
...
\begin{proof} \ 
  ...
  \begin{enumerate}
    ...
    \item Soundness. Returns exactly on the soundeness proof in the above section, when here we applay also the idea that if the inputs passes the validitly tests in lines $1-4$ with probability grater than $1-\Theta(\frac{m\cdot d}{1})$ then there is a valid input which is close enought to the given input and by conditining on quring only points that both of them agree on.  
  \end{enumerate}
\end{proof}
