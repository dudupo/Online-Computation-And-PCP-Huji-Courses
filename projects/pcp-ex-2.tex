...
\paragraph{The reduction.} Define the Bipartite graph $G = (L,R,E)$. Associate the left vertices with the variables and the right with the closures. Define $\{u,v\}$ to be an edge if the literal which associate with the vertex $u$ is in the closure associate with vertex $v$. For the alphabet take $\Sigma = \{0 , 1\}$. For any right vertex $v\in R$ define $\Sigma_{v} = \{1\}$ and for any left vertex $u$ define $\Sigma_{u} = \{1,0\}$. Finally define $c_{e}$ for $ e = \left\{ v \in R, u \in L \right\}$ to be the identity if the variable associate with $u$ appears in it's normal formal inside the closure (i.e not negative), otherwise define $c_{e}$ to be the flipping operator.

\paragraph{}

For example, assume that $v$ associate with $x\vee \bar{y} \vee z $ and let $u_{1},u_{2}$ be the vertices associated with $x$ and $y$. Then $\Sigma_{v} = \{ 1 \}$, $\Sigma_{u_{1}}=\Sigma_{u_{2}}=\{1,0\}$ and $c_{u_{1},v}A_{u_{1}} = A_{u_{1}}$, $c_{u_{2},v}A_{u_{2}} = \bar{A}_{u_{2}}$. Observes that a labeling $A$ in which $c_{u,v}A_{u}=A_{v}$ corresponds to an assignment in which the variable associate with $u$ satisfies the closure associated with $v$.  
...
\paragraph{Completeness. } Suppose that $\varphi \in \text{E3-CNF-SAT}$ and let $x \in \mathbb{F}_{2}^{*}$ be the assignment that satisfies $\varphi$. That it, $\varphi\left( x \right) = \mathbf{True}$. Let $A$ be the labeling that sets for any vertex on the left the bit matched to that literal by $x$. By the fact that $x$ satisfies $\varphi$ any closure in $\varphi$ is satisfied by $x$ and therefore each of the right vertices (closures) see on his local view at least one neighbor $u$ such $c_{u,v}A_{u} = 1 = A_{v}$ otherwise no literal in the support of $u$-closure satisfies him in contrast to the assumption that $x$ is satisfying $\varphi$.


In addition by the definition of the construction any pair of connected vertices satisfies the edge restriction. 
