...
& \le \prb{ u,v \text{ s.t } \expp{ \cdot } \ge \delta} \cdot 1 + \prb{ u,v \text{ s.t } \expp{\cdot } \le \delta}  \cdot \delta \\ 
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  Thus in total the labeling satisfies $\delta \cdot \left(\frac{1}{2} \log_{1-2\varepsilon}\left( \delta^{3} \right)\right)^{-2}\cdot 3\left( \frac{\delta}{2} \right)^{2} $ of the constraints. 
\section{Part 3.}
