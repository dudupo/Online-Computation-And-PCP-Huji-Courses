...
On the other hand, run in which the optimal gain $T\mu+\frac{1}{2}\sqrt{T}$ might occur with positive probability. Using that event, we infer that the regret has to be at least: 
  ...
  One can Compute $x$ by doing at most one iteration over the input, So the total running time is at most $O\left( m n \right)$. My guess is that if $A$ isn't restricted to hold only positive values, then the problem is equivalence to solving an arbitrary LP. 
